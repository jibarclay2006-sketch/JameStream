...
├── setup/             # App bootstrap: App.tsx (routes), config, i18n, PWA
...
- **Components used by exactly one page** belong in `pages/parts/<page>/`,
  not `components/`.
- **Advertising and third-party tracking loaders are intentionally absent.**
  Do not add operator-controlled script injection to runtime configuration.

## Fork note

This repo is an ad-free fork of `xp-technologies-dev/p-stream`. Upstream
changes should be reviewed before merging so ad, promotional, or tracking
paths are not accidentally restored.
