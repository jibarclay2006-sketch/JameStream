...
export function DownloadView({ id }: { id: string }) {
  ...
  const router = useOverlayRouter(id);
  const { t } = useTranslation();
  const originalDownloadsAvailable = false;
    ...
    <>
      ...
      <Menu.Section>
        <div className="flex flex-col gap-3 mt-2">
          <button
            type="button"
            className={`w-full rounded-lg bg-video-context-light/10 transition-colors p-4 text-left relative group ${originalDownloadsAvailable ? "hover:bg-video-context-light/20 cursor-pointer" : "opacity-50 cursor-not-allowed"}`}
            onClick={() =>
              originalDownloadsAvailable &&
              router.navigate("/download/original")
            }
            disabled={!originalDownloadsAvailable}
          >
            <div className="flex items-center gap-3">
              <Icon
                icon={Icons.FILE_ARROW_DOWN}
                className={`text-2xl ${originalDownloadsAvailable ? "text-video-context-type-accent" : "text-video-context-type-secondary"}`}
              />
              <div className="flex-1">
                ...
                <p className="text-xs text-video-context-type-secondary mt-0.5">
                  {originalDownloadsAvailable
                    ...
                    : t("player.menus.downloads.original.selfhosted")}
                </p>
              </div>
              {originalDownloadsAvailable && (
